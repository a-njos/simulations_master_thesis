...
formula_full = "Surv(t,d)~x+z1+z2+x*z1"
formula_ncc = "Surv(t,case)~x+z1+z2+x*z1+strata(setno)"
sm_formula_full = "Surv(t,d)~x+z1+z2+x*z1"
sm_formula_ncc = "Surv(t,case)~x+z1+z2+x*z1+strata(setno)"
...
for(j in seq(1,nsim)){
  ...
  imp<-mice(cohort.ncc, m = nimp, method = method.vec, 
            ...
            maxit = 20, diagnostics = FALSE, printFlag = F)
  ...
  imp<-mice(cohort.ncc, m = nimp, method = method.vec, 
            predictorMatrix = pred.mat, 
            maxit = 5, diagnostics = FALSE, printFlag = F)
  ...
  #In matched set: assign number at risk at the case's event time to every individual in the set
...
library(latex2exp)
SAVE_PLOTS=T
if(SAVE_PLOTS) pdf("figures/sim/nccintest.pdf",width=8,height=16) 
par(mfrow=c(4,2))
...
if(SAVE_PLOTS) pdf("figures/sim/nccintestz1.pdf",width=8,height=16) 
par(mfrow=c(4,2))
...
if(SAVE_PLOTS) pdf("figures/sim/nccintestz2.pdf",width=8,height=16) 
par(mfrow=c(4,2))
...
if(SAVE_PLOTS) pdf("figures/sim/nccintestxz1.pdf",width=8,height=16) 
par(mfrow=c(4,2))
...
yy_lim = c(0,3)
SAVE_PLOTS=T
if(SAVE_PLOTS) pdf("figures/sim/nccintest.pdf",width=6,height=8) 
par(mfrow=c(4,2))
...
hist(parameters[,1+npara],xlab=TeX("$\\beta_x$"),main="ncc",freq = F,breaks=seq(-.9,2.9,by=b_by),ylim=yy_lim)
...
b_by=0.05
yy_lim = c(0,3)
if(SAVE_PLOTS) pdf("figures/sim/nccintestxz1.pdf",width=6,height=8) 
par(mfrow=c(4,2))
hist(parameters[,4],xlab=TeX("$\\beta_{xz_1}$"),main="full cohort",freq = F,breaks=seq(b_from,b_to,by=b_by),ylim=yy_lim)
hist(parameters[,4+npara],xlab=TeX("$\\beta_{xz_1}$"),main="ncc",freq = F,breaks=seq(-1,2,by=b_by),ylim=yy_lim)
