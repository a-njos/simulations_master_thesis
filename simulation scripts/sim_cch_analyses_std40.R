...
for(j in seq(1,nsim)){
  ...
  #===============================
  cohort.caco = readRDS(file=paste0(filepath,"cohort.super.ext.caco",j,".rds"))
  ...
  for (k in 1:nimp){
    model=cch(as.formula(formula),data=complete(imp,k),subcoh=~subco.super.ext, id=~id, method="LinYing", cohort.size=n)
    models[[k]] = model
  }
  ...
  my.sampfrac = sum(cohort.caco$subco.super.ext==1)/n
  
  #perform the imputation
  imp <- smcfcs.casecohort(cohort.caco,smformula=sm_formula_caco,sampfrac=my.sampfrac,in.subco="subco.super.ext",
                           method=method.vec,predictorMatrix=pred.mat,m=nimp,numit=n.it,rjlimit=10000,noisy=FALSE)
  ...
  #===============================
  cohort.caco= readRDS(file=paste0(filepath,"cohort.super.ext.caco",j,".rds")) #this is 'cohort.ncc'
  ...
  models <- vector("list", nimp)
  for (k in 1:nimp){
    model=cch(as.formula(formula),data=imp$impDatasets[[k]],subcoh=~subco.super.ext, id=~id, method="LinYing", cohort.size=n)
    models[[k]] = model
  }
...
yy_lim = c(0,6)
SAVE_PLOTS=T
if(SAVE_PLOTS) pdf("figures/sim/cchstd40est.pdf",width=6,height=9) 
par(mfrow=c(4,2))
