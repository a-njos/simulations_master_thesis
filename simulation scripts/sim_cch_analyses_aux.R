...
sm_formula = "Surv(t,d)~x+z1+z2+v"

formula_prentice = "Surv(entertime,t,d)~x+z1+z2"
sm_formula_prentice = "Surv(entertime,t,d)~x+z1+z2+v"


predictors_aprx = c("z1","z2","v","d","chaz")
predictors_rs = c("z1","z2","v")
#-------------------------------
#===============================
# Run analyses 
#===============================
...
for(j in seq(1,nsim)){
  ...
  my.sampfrac = sum(cohort.caco$subco.super)/n
  
  #perform the imputation
  imp <- smcfcs.casecohort(cohort.caco,smformula=sm_formula_prentice,sampfrac=my.sampfrac,in.subco="subco.super",
                           method=method.vec,predictorMatrix=pred.mat,m=nimp,numit=100,rjlimit=10000,noisy=FALSE)
  
  # obtain estimates across the imputed data sets and combine using Rubin's Rules
  impobj <- imputationList(imp$impDatasets)
  models <- with(impobj, coxph(as.formula(paste0(formula_prentice,"+cluster(id)"))))
  coef = MIcombine(models)$coefficients
  se = sqrt(diag(MIcombine(models)$variance))
