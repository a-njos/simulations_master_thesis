...
# ------------------------------
rm(list=ls())
setwd("/Users/aleksandernjos/h2019/master/master_njos")
# ------------------------------
# ------------------------------
# GENERATE FULL-COHORT DATA
# ------------------------------
# ------------------------------
sim_setup = "standard"     #  c("standard","aux","int","nocorr")
controls=""
sim_idx = 1
filepath = paste0("simulations/",sim_setup[sim_idx],controls,"/")
nsim = 1000
...
#beta.x=ifelse(sim_setup[sim_idx]=="nocorr",1,0.5)    # log hazard ratio for X
beta.x=1   # log hazard ratio for X
beta.z1=1   # log hazard ratio for  binary Z1
beta.z2=0.5     # log hazard ratio for continous Z2
beta_int = ifelse(sim_setup[sim_idx]=="int",0.5,0)   # interaction between X and Z1  
...
if(sim_setup[sim_idx]=="nocorr"){
  b_x=0 
  c_x=0 
}else if(sim_setup[sim_idx]=="strongcorr"){
  b_x=0.75
  c_x=0.75
}

eta_v = 0.8 #  Gaussian standard deviation for auxilliary 
...
kappa = 4  # Weibull baseline shape for event of interest
...
for(j in seq(1,nsim)){
  ...
  cor(matrix(c(x,v),ncol=2,byrow=F))
  cor(matrix(c(x,z1,z2,v),ncol=4))
  
  # ------------------------------
  # Generate potential event times
  u=runif(n,0,1)
  t.event=(-log(u)*(1/lambda)*exp(-(beta.x*x+beta.z1*z1+beta.z2*z2+x*z1*beta_int)))^(1/kappa)
  mean(t.event)
  plot(density(t.event),xlim=c(0,50))
  
  # ------------------------------
  # Generate potential drop-out time
  u=runif(n,0,1)
  t.drop=(-log(u)*(1/lambda_drop)*exp(-(beta_x_drop*x+beta_z1_drop*z1+beta_z2_drop*z2)))^(1/kappa_drop)
  mean(t.drop)
  plot(density(t.drop))
  ...
  par(mfrow=c(1,3))
  hist(t.event[t.event<close.time])
  hist(t.drop[t.drop<close.time])
  hist(cohort$t,main=paste0("histogram of cohort ",j))
  SAVE_PLOTS=F
  if(SAVE_PLOTS) pdf("figures/sim/cohort1.pdf",width=8 ,height = 6) 
  par(mfrow=c(1,3))
  hist(t.event[t.event<close.time],main="event")
  hist(t.drop[t.drop<close.time],main="non-administrative censoring")
  hist(cohort$t,main=paste0("histogram of cohort ",j))
  if (SAVE_PLOTS) dev.off()
  par(mfrow=c(1,1))
  
  saveRDS(cohort,file=paste0(filepath,"cohort",j,".rds")) # save full cohort
  
  # ------------------------------
  # ------------------------------
  # GENERATE NCC SUPERSET 
  # ------------------------------
  # ------------------------------
  ...
  cohort.super.ext$x<-ifelse(cohort.super.ext$id%in%ncc$id,cohort.super.ext$x,NA)
  sum(is.na(cohort.ncc$x))
  sum(is.na(cohort.super$x))
  sum(is.na(cohort.super.ext$x))
  
  library(survival)
  if(sim_setup[sim_idx]=="int"){
    print(coxph(Surv(t,d)~x+z1+z2+x*z1,data=cohort))
    print(coxph(Surv(t,case)~x+z1+z2+x*z1+strata(setno),data=ncc))
    print(coxph(Surv(t,case)~x+z1+z2+x*z1+strata(setno),data=ncc.super))
    print(coxph(Surv(t,case)~x+z1+z2+x*z1+strata(setno),data=ncc.super.ext))
  }else{
    print(coxph(Surv(t,d)~x+z1+z2,data=cohort))
    print(coxph(Surv(t,case)~x+z1+z2+strata(setno),data=ncc))
    #print(coxph(Surv(t,d)~x+z1+z2,data=cohort.ncc))
    #print(coxph(Surv(t,case)~x+z1+z2+strata(setno),data=cohort.super))
    #print(coxph(Surv(t,d)~x+z1+z2,data=cohort.super))
    print(coxph(Surv(t,case)~x+z1+z2+strata(setno),data=ncc.super))
    #print(coxph(Surv(t,d)~x+z1+z2,data=ncc.super))
    print(coxph(Surv(t,case)~x+z1+z2+strata(setno),data=ncc.super.ext))
  }
  
  # ------------------------------
  # save NCC data sets
  saveRDS(ncc,file=paste0(filepath,"ncc",j,".rds"))  # ncc sample
  
  saveRDS(cohort.ncc,file=paste0(filepath,"cohort.ncc",j,".rds")) # NCC within full cohort
  
  saveRDS(cohort.super,file=paste0(filepath,"cohort.super.ncc",j,".rds")) # NCC within superset ncc
  
  saveRDS(cohort.super.ext,file=paste0(filepath,"cohort.super.ext.ncc",j,".rds")) # NCC within superset extended ncc
  ...
  cohort.caco$subco.super.ext <- c(rep(1,n.subco.super.ext),rep(0,n-n.subco.super.ext))
  # ------------------------------
  ...
  cohort.super.caco=cohort.caco[cohort.caco$subco.super==1|cohort.caco$d==1,]
  
  cohort.super.ext.caco=cohort.caco[cohort.caco$subco.super.ext==1|cohort.caco$d==1,]
  ...
  cohort.super.caco$entertime=ifelse(cohort.super.caco$d==1 & cohort.super.caco$subco.super==0,cohort.super.caco$t-0.001,0) #this is used in the intermediate MI method
  
  cohort.super.ext.caco$entertime=ifelse(cohort.super.ext.caco$d==1 & cohort.super.ext.caco$subco.super.ext==0,cohort.super.ext.caco$t-0.001,0) 
  
  library(survival)
  if(sim_setup[sim_idx]=="int"){
    print(coxph(Surv(t,d)~x+z1+z2+x*z1,data=cohort))
    print(cch(Surv(t,d)~x+z1+z2+x*z1,
              data=caco, subcoh=~subco, id=~id, method="LinYing", cohort.size=n))
    #print(coxph(Surv(entertime,t,d)~x+z1+z2+x*z1+cluster(id),data=caco))
  }else{
    print(coxph(Surv(t,d)~x+z1+z2,data=cohort))
    print(cch(Surv(t,d)~x+z1+z2,
              data=caco, subcoh=~subco, id=~id, method="LinYing", cohort.size=n))
  }
  ...
  saveRDS(cohort.super.caco,file=paste0(filepath,"cohort.super.caco",j,".rds")) 
  
  saveRDS(cohort.super.ext.caco,file=paste0(filepath,"cohort.super.ext.caco",j,".rds")) 
}
